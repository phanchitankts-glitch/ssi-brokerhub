...
# ---- BẢNG CHỈ SỐ THỊ TRƯỜNG ĐÃ NÂNG CẤP DÙNG API ----
st.markdown("---")

@st.cache_data(ttl=60) # Cache 60s để tránh quá tải API
def get_index_realtime(ticker, fallback_value):
    try:
        idx = yf.Ticker(ticker)
        current_price = idx.fast_info['lastPrice']
        
        # Lấy điểm mở cửa để tính phần trăm tăng giảm
        ...
        # Nếu Yahoo Finance lỗi, trả về số dự phòng hợp thời đại (>1800 điểm)
...
with col_idx1:
    st.metric("VN-INDEX", vni_score, vni_change)
with col_idx2:
    st.metric("VN30-INDEX", vn30_score, vn30_change)
with col_idx3:
    st.metric("HNX-INDEX", "240.20", "2.10 (0.88%)") # HNX ít cập nhật trên YF nên fix cứng
with col_idx4:
    st.metric("Thanh khoản HOSE", "28,500 Tỷ VNĐ", "Tăng 12%")
st.markdown("---")
...
with col_main1:
    with st.container(border=True):
        ...
        # ----------------------------------------------------
        # SỬ DỤNG 100% YFINANCE THAY THẾ CHO VNSTOCK
        # ----------------------------------------------------
        real_price = 0
        df_historical = None
        ...
        with st.spinner("Đang đồng bộ dữ liệu Real-time & Chart..."):
            try:
                # Dùng yfinance kéo dữ liệu 6 tháng để vẽ Chart và tính RSI
...
with col_main2:
    # VẼ BIỂU ĐỒ NẾN CANDLESTICK CHUYÊN NGHIỆP
    if df_historical is not None and not df_historical.empty:
        ...
        fig.update_layout(
            title=f"Biểu đồ biến động giá 6 tháng - {selected_ticker}",
            yaxis_title="Giá (VNĐ)",
...
# ----------------------------------------------------
# PHÂN TÍCH TỰ ĐỘNG DỰA TRÊN CHỈ SỐ RSI THẬT
# ----------------------------------------------------
if real_rsi != "N/A":
...
with col_f1:
    st.metric("Chỉ số ROE", real_roe)
with col_f2:
    st.metric("EPS (Trượt)", real_eps)
with col_f3:
    st.metric("P/E Forward", real_pe)
with col_f4:
    st.metric("Chỉ số RSI (14)", real_rsi)
...
if not df_customers.empty:
    ...
    if st.button("Xác nhận gửi tín hiệu Smart OTP", type="primary"):
        if selected_customers:
            st.success(f"Yêu cầu thành công: Đã gửi thông báo khuyến nghị {selected_ticker} đến {len(selected_customers)} khách hàng qua ứng dụng SSI iBoard Pro.")
        else:
